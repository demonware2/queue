...
const logToConsole = (message, type = 'info') => {
  ...
  try {
    ...
    const logFile = path.join(logDir, `anggaranswakelola-${today}.log`);
...
async function transform(tahun) {
  ...
  try {
    ...
    const [rows] = await connection.query(
      `SELECT * FROM api_paketanggaranswakelola 
       WHERE tahun = ? 
    ...
    if (rows.length === 0) {
      throw new Error("No data found in api_paketanggaranswakelola");
    }
    ...
    await connection.query(
      'DELETE FROM paketanggaranswakelola WHERE tahun_anggaran_dana = ?',
      [tahun]
    );
    
    if (revisi.length > 0) {
      await connection.query(
        `INSERT INTO paketanggaranswakelola 
  ...
  try {
    ...
    await connection.query(
      `DELETE FROM paket_sirupmak_tm 
       WHERE jenispaket = 'SWAKELOLA' AND tahun_anggaran_dana = ?`,
      [tahun]
    );
...
async function update() {
  try {
    logToConsole('Starting AnggaranSwakelola update process', 'info');
    ...
    for (const tahun of tahuns) {
      ...
      for (let retries = 0; retries < maxRetries; retries++) {
        try {
          ...
          if (isAllowed) {
            const url = `https://isb.lkpp.go.id/isb-2/api/145b92a6-09e6-4a70-b4b0-1462172a41f4/json/9454/RUP-PaketAnggaranSwakelola/tipe/4:12/parameter/${tahun}:L15`;
            ...
            try {
              ...
              const [countResult] = await connection.query(
                'SELECT COUNT(*) as total FROM api_paketanggaranswakelola'
              );
              const totalRows = countResult[0].total;
              
              if (totalRows >= 14) {
                const [oldestRow] = await connection.query(
                  'SELECT id FROM api_paketanggaranswakelola ORDER BY created ASC LIMIT 1'
                );
                
                if (oldestRow.length > 0) {
                  await connection.query(
                    'DELETE FROM api_paketanggaranswakelola WHERE id = ?',
                    [oldestRow[0].id]
                  );
                }
              }
              ...
              await connection.query(
                'INSERT INTO api_paketanggaranswakelola (request, response, tahun) VALUES (?, ?, ?)',
              ...
              try {
                ...
                if (deleteRedisResult) {
                  logToConsole(`Delete Redis successful!`, 'info');
                } else {
                  logToConsole(`Delete Redis failed`, 'error');
                }
...
async function main() {
  ...
  if (command === 'update') {
    ...
    logToConsole('Usage: node anggaranSwakelola.js update [--task-id=X]', 'info');
    process.exit(1);
  }
}
